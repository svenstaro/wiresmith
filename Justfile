...
interactive:
    ...
    podman run -t --replace --rm --name wiresmith3 -l testcontainer -d -p 33333:33333/udp --hostname wiresmith3 --network slirp4netns:allow_host_loopback=true --cap-add SYS_ADMIN,NET_ADMIN -v ./target/debug/wiresmith:/usr/bin/wiresmith --tz UTC --entrypoint /sbin/init docker.io/archlinux
